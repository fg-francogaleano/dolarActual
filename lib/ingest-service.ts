...
export async function ingestNews() {
  ...
  const validNews = rawNews.filter(item => {
    const hasImage = item.image && item.image !== "null" && item.image.trim() !== "";
    return hasImage;
  });

  if (validNews.length === 0) {
    console.log("⚠️ [Ingest Service] Se obtuvieron noticias pero ninguna tenía imagen válida. No se guardará nada.");
    return { status: 'Warning', message: 'No valid news with images found' };
  }

  console.log(`🔍 [Ingest Service] Filtrado: ${rawNews.length} originales -> ${validNews.length} con imagen.`);

  const operations = validNews.map(item => ({
    updateOne: {

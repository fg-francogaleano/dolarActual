...
const Header: React.FC = () => {
  ...
  // --- CONFIGURACIÓN DE MENÚS (Igual que antes) ---
  const quotesMenu = {
  ...
  // --- ESTILOS SEMÁNTICOS ---
  // Usamos text-brand-600 para activo y hover:bg-accent para hover
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container mx-auto px-4">
        <div className="flex h-16 items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center space-x-1">
            ...
            >
              ...
              <div 
                className={`absolute left-0 top-full mt-1 w-[600px] bg-popover border border-border rounded-lg shadow-xl z-50 grid grid-cols-3 gap-4 p-5 transition-all duration-200 ${
                  isQuotesOpen ? "opacity-100 visible translate-y-0" : "opacity-0 invisible -translate-y-2"
                }`}
              >
                {quotesMenu.sections.map((section) => (
                  <div key={section.title}>
                    <h4 className="font-bold text-xs text-muted-foreground mb-2 uppercase tracking-wide border-b border-border pb-1">
                      {section.title}
                    </h4>
                        ...
                        <li key={item.path}>
                          <Link 
                            ...
                            className={`block px-2 py-1.5 text-sm rounded-md transition-colors ${
                              pathname === item.path 
                                ? "text-brand-600 bg-brand-50 dark:text-brand-400 dark:bg-brand-900/20 font-medium"
            ...
            >
              ...
              <div 
                className={`absolute left-0 top-full mt-1 w-48 bg-popover border border-border rounded-lg shadow-xl z-50 py-2 transition-all duration-200 ${
                  isNewsOpen ? "opacity-100 visible translate-y-0" : "opacity-0 invisible -translate-y-2"
                }`}
              >
                <div className="px-4 py-2 text-xs font-bold text-muted-foreground uppercase tracking-wider border-b border-border mb-1">
                  {t("news.sections")}
                </div>
                {newsMenu.map((item) => (
                  <Link 
                ...
                <div className="mt-1 pt-2 border-t border-border text-center">
                   <Link 
                     href="/noticias" 
                     onClick={() => setIsNewsOpen(false)}
                     className="block px-4 py-2 text-xs font-bold text-brand-600 hover:underline dark:text-brand-400"
                   >
                     {t("news.all")} →
                   </Link>
                </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            <Button variant="ghost" size="icon" className="lg:hidden text-foreground/80 hover:text-foreground" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
        ...
        {/* --- MOBILE NAVIGATION --- */}
        {mobileMenuOpen && (
          <nav className="lg:hidden py-4 border-t border-border h-[calc(100vh-64px)] overflow-y-auto">
            {/* ... lógica móvil similar usando colores semánticos ... */}
            {/* Simplificado para brevedad, sigue los mismos patrones que Desktop */}
             <Link href="/" onClick={() => setMobileMenuOpen(false)} className="block px-3 py-3 text-base font-medium text-foreground border-b border-border hover:bg-accent">
              {t("nav.home")}
            </Link>
            {/* ... (resto de items con text-foreground, border-border, etc) ... */}
          </nav>
        )}
      </div>
    </header>

...
export default function QuoteDetailContent({
  ...
  // 1. Hook de datos en vivo (Actualiza cada 30 segundos)
  const { quotes } = useLiveQuotes({ refreshInterval: 30000 });

  // 2. Mapeo Inteligente: 
  // Buscamos los valores nuevos en el array vivo. Si aún no cargaron, usamos los datos iniciales.
  const featured = quotes.find((q) => q.id === initialFeatured.id) || initialFeatured;
  const related = initialRelated.map(
    (r) => quotes.find((q) => q.id === r.id) || r
  );

  // 3. Reloj Interno (Tick) para forzar el re-render de la fecha relativa
  ...
  const isNeutral = (featured.variacion || 0) === 0;

  // --- Helpers de Traducción ---
  const getTranslatedName = (item: UnifiedQuote) => {
    const translationKey = `quotations.${item.id}`;
    const translated = t(translationKey);
    // Fallback por si no existe traducción
    return translated === translationKey ? item.nombre : translated;
  };

  // Prevenir Hydration Mismatch
  const fechaRelativa = (isMovile: boolean) => {
    const aux = isMounted 
    ? formatDateShort(featured.fechaActualizacion, isMovile) 
    : <Loader2 className="w-5 h-5 animate-spin text-muted-foreground" />;
    return aux
  }
  const fechaRelativ = isMounted 
    ? formatDateShort(featured.fechaActualizacion) 
    : <Loader2 className="w-5 h-5 animate-spin text-muted-foreground" />;
      ...
      <div className="bg-card rounded-xl shadow-xl p-8 mb-12">
        ...
        <p className="text-muted-foreground text-sm flex mt-8">
          ...
          <span className="hidden md:inline ml-2">
            {/* {formatDateShort(fechaRelativa, false, language)} */}
            {fechaRelativa(false)}
          </span>
...
// }: QuoteDetailContentProps) {
//   const { t, language } = useLanguage();
// console.log(related,"QuoteDetailContent")
//   const isPositive = (featured.variacion || 0) > 0;
//   const isNegative = (featured.variacion || 0) < 0;
